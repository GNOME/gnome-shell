...
PKG_CACHE_DIR=${XDG_CACHE_HOME:-$HOME/.cache}/gnome-shell
...
if echo $XDG_CURRENT_DESKTOP | grep -q Classic; then
  MIGRATION_GUARD=$PKG_CACHE_DIR/classic-overrides-migrated
  OVERRIDE_SCHEMA=org.gnome.shell.extensions.classic-overrides
else
  MIGRATION_GUARD=$PKG_CACHE_DIR/gnome-overrides-migrated
