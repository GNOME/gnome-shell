...
var BaseAppView = GObject.registerClass({
    ...
    selectApp(id) {
        if (this._items.has(id) && this._items.get(id).mapped) {
            this._selectAppInternal(id);
        } else if (this._items.has(id)) {
            // Need to wait until the view is mapped
            let signalId = this._items.get(id).connect('notify::mapped',
                actor => {
                    if (actor.mapped) {
...
var AllView = GObject.registerClass({
    ...
    _redisplay() {
        ...
        addedIDs.forEach(id => {
            ...
            let index = Util.insertSorted(this._orderedItems, icon, this._compareItems);
    ...
    _updateIconOpacities(folderOpen) {
        for (let [id, icon] of this._items) {
            let opacity;
