...
function loadSymbolicIcon(cr, name, size, themeNode) {
    let icon = new Gio.ThemedIcon({ name: name });
    let scale = St.ThemeContext.get_for_stage(global.stage).scale_factor;

    let textureCache = St.TextureCache.get_default();

    let surface = textureCache.load_gicon_to_cairo_surface (themeNode, icon, size, scale);
...
var BatteryIcon = new Lang.Class({
    ...
    _drawStaticIcon(cr, size, themeNode, iconName) {
        let surface = loadSymbolicIcon(cr, iconName, size, themeNode);
    ...
    _drawDynamicBattery(cr, size, themeNode, charging) {
        ...
        let rectT = BATTERY_TOP / scale * size;
        let rectB = BATTERY_BOTTOM / scale * size;
        let rectL = BATTERY_LEFT / scale * size;
        let rectR = BATTERY_RIGHT / scale * size;

        rectT += (rectB - rectT) * frac;

        let surface = loadSymbolicIcon(cr, fgName, size, themeNode);
        cr.setSourceSurface(surface, 0, 0);
