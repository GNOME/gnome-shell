...
function makeChatMessageFromTpMessage(tpMessage, direction) {
...
function makeChatMessageFromTplEvent(tplTextEvent) {
    let direction =
...
class ChatSource extends MessageTray.Source {
    ...
    _displayPendingMessages(logManager, result) {
        ...
        let logMessages = events.map(e => makeChatMessageFromTplEvent(e));
        ...
        for (let i = 0; i < pendingTpMessages.length; i++) {
            ...
            pendingMessages.push(makeChatMessageFromTpMessage(message,
    ...
    _messageReceived(channel, message) {
        ...
        message = makeChatMessageFromTpMessage(message,
            NotificationDirection.RECEIVED);
