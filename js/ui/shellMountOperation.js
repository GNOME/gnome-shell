...
var ShellMountPasswordDialog = class extends ModalDialog.ModalDialog {
	...
	this._passwordEntry.secondary_icon = this._workSpinner.actor;
        ...
        if (flags & Gio.AskPasswordFlags.TCRYPT) {
            ...
            if (rtl) {
                layout.attach(this._pimEntry, 0, 1, 1, 1);
                layout.attach(this._pimLabel, 1, 1, 1, 1);
            } else {
                layout.attach(this._pimLabel, 0, 1, 1, 1);
                layout.attach(this._pimEntry, 1, 1, 1, 1);
            }

            this._pimErrorMessageLabel = new St.Label({ style_class: 'prompt-dialog-password-entry',
                                                        text: _("The PIM must be a number of empty."),
                                                        visible: false });
    ...
    _onKeyfilesCheckboxClicked() {
        if (this._keyfilesCheckbox.actor.checked) {
            this._keyfilesLabel.show();
            this.setButtons(this._usesKeyfilesButtons);
        } else {
            this._keyfilesLabel.hide();
            this.setButtons(this._defaultButtons);
        }
