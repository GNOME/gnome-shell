...
var AppSwitcherPopup = new Lang.Class({
    ...
    _windowActivated(thumbnailList, n) {
        Main.activateWindow(thumbnailList.icon.cachedWindows[n]);
        this.destroy();
    },
    ...
    _windowRemoved(thumbnailList, n) {
        ...
        if (this._thumbnailsFocused) {
            ...
            if (thumbnailList.icon.cachedWindows.length == 1) {
                this._select(this._selectedIndex);
                return;
            }

            let newIndex = thumbnailList.icon.cachedWindows.length - 1;

            if (n < this._currentWindow)
                newIndex = this._currentWindow - 1;
            else if (n == this._currentWindow && n != thumbnailList.icon.cachedWindows.length)
