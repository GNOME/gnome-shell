...
class WindowSwitcherPopup extends SwitcherPopup.SwitcherPopup {
    _init() {
        ...
        const mode = this._settings.get_enum('app-icon-mode');
    ...
    _closeWindow(windowIndex) {
        const windowIcon = this._items[windowIndex];
        if (!windowIcon)
            return;
...
class WindowIcon extends St.BoxLayout {
    _init(window, mode) {
        ...
        const mutterWindow = this.window.get_compositor_private();
        let size;
...
class WindowSwitcher extends SwitcherPopup.SwitcherList {
    _init(windows, mode) {
        ...
        for (let i = 0; i < windows.length; i++) {
            const win = windows[i];
            const icon = new WindowIcon(win, mode);
    ...
    vfunc_get_preferred_height(forWidth) {
        ...
        const spacing = this.get_theme_node().get_padding(St.Side.BOTTOM);
        const [labelMin, labelNat] = this._label.get_preferred_height(-1);
