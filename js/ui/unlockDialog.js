...
export const UnlockDialog = GObject.registerClass({
    ...
    _init(parentActor) {
        ...
        Main.timeLimitsManager.connectObject(
            'notify::state', () => this._updateAuthPromptLockability(),
            'notify::daily-limit-time', () => this._updateAuthPromptLockability(),
            this);
        this._updateAuthPromptLockability();
    ...
    _updateAuthPromptLockability() {
        if (this._authPrompt) {
            const dailyLimitReached = Main.timeLimitsManager.state ===
                TimeLimitsState.LIMIT_REACHED;
            this._authPrompt.updateLockability(dailyLimitReached);
        }
    }
