...
var WorkspaceAnimationController = class {
    ...
    _finishWorkspaceSwitch(switchData) {
        ...
        for (const record of switchData.monitors)
            record.clipBin.destroy();
    ...
    animateSwitchWorkspace(from, to, direction, onComplete) {
        if (this._switchData) {
            for (const record of this._switchData.monitors)
        ...
        for (const record of this._switchData.monitors) {
            const fromWs = record.workspaces[from];
            const toWs = record.workspaces[to];

            record.container.x = -fromWs.x;
            record.container.y = -fromWs.y;
            ...
            if (record.index === Main.layoutManager.primaryIndex) {
                params.onComplete = () => {
                    this._finishWorkspaceSwitch(this._switchData);
                    onComplete();
                };
    ...
    _switchWorkspaceUpdate(tracker, progress) {
        ...
        for (const record of this._switchData.monitors) {
            ...
            const p = this._interpolateProgress(progress, record);
            ...
            record.container.set_position(xPos, yPos);
        }
    }
    ...
    _switchWorkspaceEnd(tracker, duration, endProgress) {
        ...
        for (const record of this._switchData.monitors) {
            const newWs = record.workspaces[index];
