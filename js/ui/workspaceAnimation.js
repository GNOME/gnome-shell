...
var WorkspaceAnimationController = class {
    ...
    _prepareWorkspaceSwitch(workspaces) {
        ...
        for (const monitor of Main.layoutManager.monitors) {
            ...
            record.bgManager = new Background.BackgroundManager({
                container: record.background,
    ...
    animateSwitch(from, to, direction, onComplete) {
        ...
        for (const monitorInfo of Object.values(this._switchData.monitors)) {
            const fromWs = this._findWorkspaceInfoByIndex(from, monitorInfo.index);
            const toWs = this._findWorkspaceInfoByIndex(to, monitorInfo.index);
    ...
    _findWorkspaceInfoByIndex(index, monitorIndex) {
        const workspace = global.workspace_manager.get_workspace_by_index(index);
        const monitorInfo = this._switchData.monitors[monitorIndex];
        return monitorInfo.workspaces.find(ws => ws.ws == workspace);
    }
    ...
    _switchWorkspaceBegin(tracker, monitor) {
        ...
        if (this._switchData && this._switchData.gestureActivated) {
            ...
            const index = this._findClosestWorkspaceIndex(progress);
            const ws = this._findWorkspaceInfoByIndex(index, monitor);
            cancelProgress = this._getProgressForWorkspace(ws, monitor);
        } else {
            this._prepareWorkspaceSwitch();

            const activeIndex = workspaceManager.get_active_workspace_index();
            const ws = this._findWorkspaceInfoByIndex(activeIndex, monitor);
    ...
    _switchWorkspaceEnd(tracker, duration, endProgress) {
        ...
        for (const monitorInfo of Object.values(this._switchData.monitors)) {
            const newWs = this._findWorkspaceInfoByIndex(index, monitorInfo.index);
