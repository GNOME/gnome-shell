...
class Panel extends St.Widget {
    ...
    _onBackgroundChanged(bgManager) {
        ...
        if (success) {
            ...
            // Also make sure that if one area is very bright we always call the image noisy
            // because we never want to show white text on (partly) bright backgrounds.
            let additionalCheckAreas =
                [{ start: 0, width: BACKGROUND_CHECK_AREA_WIDTH },
                 { start: this._centerBox.get_transformed_position()[0],
                   width: this._centerBox.get_width() },
                 { start: this._rightBox.get_transformed_position()[0],
                   width: this._rightBox.get_width() }];

            for (let point of additionalCheckAreas) {
                let [boxSuccess, boxBgIsNoisy, boxBgIsDark, boxBgIsBright] =
                    bgManager.getCharacteristicsForArea(point.start, this.y, point.width, this.get_height());
