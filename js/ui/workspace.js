...
var WindowCloneLayout = GObject.registerClass({
    ...
    set max_child_width(w) {
        ...
        // don't call this.layout_changed() here to avoid allocation loops
    }
});
...
//            layout_manager: new WindowCloneLayout(),
    ...
    vfunc_get_preferred_width(forHeight) {
        const [minWidth, natWidth] =
            this._cloneContainer.get_preferred_width(forHeight);

        return this.get_theme_node().adjust_preferred_width(minWidth, natWidth);
    }

    vfunc_get_preferred_height(forWidth) {
        const [minHeight, natHeight] =
            this._cloneContainer.get_preferred_height(forWidth);
    ...
    setMaxChromeWidth(maxWidth) {
       // this.layout_manager.max_child_width = maxWidth;
    }
...
class Workspace extends St.Widget {
    ...
    _updateWindowPositions(flags) {
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            clone.setMaxChromeWidth(Math.round(maxChromeWidth));
