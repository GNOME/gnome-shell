...
function sameDay(datetimeA, datetimeB) {
    return Math.abs(datetimeA.difference(datetimeB)) <= GLib.TIME_SPAN_DAY &&
        (datetimeA.get_day_of_year() == datetimeB.get_day_of_year());
}
...
var Calendar = GObject.registerClass({
    ...
    _buildHeader() {
        ...
        let iter = newDateTimeNow({ hour: 12, minute: 0, seconds: 0 });
        for (let i = 0; i < 7; i++) {
