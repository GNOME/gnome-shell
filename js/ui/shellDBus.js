...
var GnomeShell = new Lang.Class({
    ...
    GrabAcceleratorAsync(params, invocation) {
        let [accel, modeFlags, ignoreKeyRepeat] = params;
        let sender = invocation.get_sender();
        let grabFlags = ignoreKeyRepeat ? Meta.KeyBindingFlags.IGNORE_AUTOREPEAT
    ...
    GrabAcceleratorsAsync(params, invocation) {
        ...
        for (let i = 0; i < accels.length; i++) {
            let [accel, modeFlags, ignoreKeyRepeat] = accels[i];
             let grabFlags = ignoreKeyRepeat ? Meta.KeyBindingFlags.IGNORE_AUTOREPEAT : Meta.KeyBindingFlags.NONE;
