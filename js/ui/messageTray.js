...
var Source = GObject.registerClass({
    ...
    notify(propName) {
        if (propName instanceof Notification) {
            try {
                throw new Error('Source.notify() has been moved to Source.addNotification()' +
                                'this code will break in the future');
            } catch (e) {
                logError(e);
                this.addNotification(propName);
                return;
            }
        }
...
var MessageTray = class MessageTray {
    ...
    _addSource(source) {
        ...
        obj.promptId = source.connect('notification-added', this._onNotificationPrompt.bind(this));
