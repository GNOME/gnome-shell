...
function findAppFromInhibitor(inhibitor) {
    ...
    try {
        ...
        console.warn(`gnome-session gave us a dead inhibitor: ${inhibitor.get_object_path()}`);
        return null;
    }
...
class EndSessionDialog extends ModalDialog.ModalDialog {
    ...
    async _triggerOfflineUpdateReboot() {
        ...
        try {
            await this._softwareOfflineUpdatesProxy.SetActionAsync('reboot');
        } catch (error) {
            console.warn(error.message);
        }
    }
    ...
    async _loadSessions() {
        ...
        if (!sessionId) {
            ...
            console.warn(`endSessionDialog: No XDG_SESSION_ID, fetched from logind: ${sessionId}`);
        }
    ...
    async OpenAsync(parameters, invocation) {
        ...
        try {
            ...
            if (this._softwareOfflineUpdatesProxy !== null)
                console.warn(`Failed to get update info from gnome-software: ${e.message}`);
