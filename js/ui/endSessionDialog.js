...
class EndSessionDialog extends ModalDialog.ModalDialog {
    _init() {
        ...
        this._weakServiceSection = new Dialog.ListSection({
            title: _('Some services are busy or have unsaved work'),
        });
        this.contentLayout.add_child(this._weakServiceSection);

        this._strongServiceSection = new Dialog.ListSection({
            title: _('Some services are preventing this action'),
        });
        this.contentLayout.add_child(this._strongServiceSection);
    ...
    _sync() {
        ...
        const hasStrongServices = this._strongServiceSection.list.get_children().length > 0;
        this._strongServiceSection.visible = hasStrongServices;
        this._confirmButtons.forEach(btn => btn.reactive = !hasStrongServices);

        const hasApplications = this._applicationSection.list.get_children().length > 0;
        this._applicationSection.visible = hasApplications && !hasStrongServices;

        const hasWeakServices = this._weakServiceSection.list.get_children().length > 0;
        this._weakServiceSection.visible = hasWeakServices && !hasStrongServices;

        const hasSessions = this._sessions.length > 0;
        this._sessionSection.visible = this._sessions.length > 0;
    }
    ...
    _updateButtons() {
        ...
        for (let i = 0; i < dialogContent.confirmButtons.length; i++) {
            ...
            this._confirmButtons.push(button);
            ...
            if (this._canRebootToBootLoaderMenu && signal === 'ConfirmedReboot') {
                ...
                this._confirmButtons.push(this._rebootButtonAlt);
            }
        }
    }
    ...
    _handleNewInhibitor(inhibitor) {
        const dialogContent = DialogContent[this._type];

        const [flags] = inhibitor.GetFlagsSync();
        const isLogout = flags & GnomeSession.InhibitFlags.LOGOUT;
        const isPoweroff = flags & GnomeSession.InhibitFlags.POWEROFF;
        const powerOffRelevant = dialogContent.isPowerOff;
        const isRelevant = isLogout || (powerOffRelevant && isPoweroff);

        if (!isRelevant) {
            // Skip inhibitors that don't inhibit actions relevant to this dialog
            return false;
        }
        ...
        if (app) {
            // App inhibitors. User is always allowed to ignore these
            ...
            const [displayName] = inhibitor.GetDisplayNameSync();
            const listItem = new Dialog.ListSectionItem({
                icon_actor: serviceIcon,
                title: displayName,
                description,
            });
            if (flags & GnomeSession.InhibitFlags.STRONG) {
                // Strong service inhibitor. User is not allowed to ignore it
                // from the GUI. Used by things like package managers and other
                // critical services that might cause damage if interrupted.
                this._strongServiceSection.list.add_child(listItem);
            } else {
                // Weak service inhibitor. User can bypass it, though they may need
                // to authenticate with Polkit first.
                this._weakServiceSection.list.add_child(listItem);
            }
        }
    ...
    _onInhibitorLoaded(inhibitor) {
        if (!this._inhibitors.includes(inhibitor)) {
            // Stale inhibitor
            return;
        }
        ...
        let keep = true;
        try {
            keep = this._handleNewInhibitor(inhibitor);
        } catch {
            // XXX -- sometimes JIT inhibitors generated by gnome-session
            // disappear while we're loading them. Don't fail in this case
            log(`gnome-session gave us a dead inhibitor: ${proxy.get_object_path()}`);
            keep = false;
        }
    ...
    async OpenAsync(parameters, invocation) {
        ...
        this._strongServiceSection.list.destroy_all_children();
        this._weakServiceSection.list.destroy_all_children();
        ...
        if (dialogContent.isPowerOff) {
            // Powering off affects other sessions, so let's list them
