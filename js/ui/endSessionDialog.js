...
class EndSessionDialog extends ModalDialog.ModalDialog {
    ...
    _onInhibitorLoaded(inhibitor) {
        ...
        if (app) {
            let appIcon = app.create_icon_texture(_ITEM_ICON_SIZE);
            let name = app.get_name();
            let [description] = inhibitor.GetReasonSync();
            this._applicationSection.addListItem(appIcon, name, description);
        } else {
    ...
    _loadSessions() {
        this._loginManager.listSessions(result => {
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                this._sessionSection.addListItem(userAvatar, userLabelText, null);
    ...
    OpenAsync(parameters, invocation) {
        ...
        this._applications = [];
        this._applicationSection.clearList();

        this._sessions = [];
        this._sessionSection.clearList();
