...
var Switch = new Lang.Class({
    ...
    _init(state, reactive, popupSwitchMenuItem) {
        this.actor = new St.BoxLayout({ style_class: 'toggle-switch',
                                        vertical: false,
                                        accessible_role: Atk.Role.CHECK_BOX,
                                        can_focus: true });

        this._popupSwitchMenuItem = popupSwitchMenuItem;
    ...
    _endDragging() {
        if (this._dragging) {
            ...
            if (!this._dragDifX) {
                this.toggle();
                this._popupSwitchMenuItem.emit('activate');
            }
            else {
                let onX = this._getSwitchOnHandleTranslationX();
                let offX = this._getSwitchOffHandleTranslationX();
                let medX = (onX + (offX - onX)/2);
                this.setToggleState(medX < this._handle.translation_x);
            }
            this._popupSwitchMenuItem.emit('toggled', this.state);
