...
class ActorTreeViewer extends St.BoxLayout {
    ...
    _showActorChildren(actor) {
        ...
        const actorData = this._actorData.get(actor);
        if (actorData.visible)
            return;

        actorData.visible = true;
        actorData.actorAddedId = actor.connect('actor-added', (container, child) => {
            this._addActor(actorData.childrenContainer, child);
        });
        actorData.actorRemovedId = actor.connect('actor-removed', (container, child) => {
            this._removeActor(child);
        });

        for (let child of actor)
            this._addActor(actorData.children, child);
    }
...
class LookingGlass extends St.BoxLayout {
    ...
    goToPage(index) {
        this._notebook.selectIndex(index);
    }
