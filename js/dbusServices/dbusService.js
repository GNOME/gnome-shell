...
var ServiceImplementation = class {
    ...
    export() {
        this._handleMethodId = this._dbusImpl.connect('handle-method-call',
            (impl, name, params, invocation) => {
                if (this._hasSignals)
                    this._trackSender(invocation.get_sender());
    ...
    unexport() {
        ...
        if (this._methodId > 0)
            this._dbusImpl.disconnect(this._methodId);
        this._methodId = 0;
    }
    ...
    _handleError(invocation, error) {
        ...
        if (error instanceof GLib.Error) {
            invocation.return_gerror(gerror);
        } else {
            let name = error.name;
