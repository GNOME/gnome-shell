...
export const AuthPrompt = GObject.registerClass({
    ...
    _initInputRow() {
        ...
        this._authListTitle = new St.Button({
            ...
            x_expand: true,
            child: new St.Label({style_class: 'login-dialog-auth-list-label'}),
            reactive: false,
            can_focus: false,
        });
    ...
    _onWebLogin(_, serviceName, introMessage, message, url, code, buttons) {
        if (this._queryingService)
            this.clear();
    ...
    _openWebLoginDialog() {
        ...
        if (!this._webLoginDialog.visible)
            this._fadeInElement(this._webLoginDialog);

        // Make authPrompt wider to display long urls
    ...
    updateSensitivity(sensitive) {
        ...
        else if (this._webLoginIntro.visible)
            authWidget = this._webLoginIntro;
        else if (this._webLoginDialog.visible)
            authWidget = this._webLoginDialog;
        else
