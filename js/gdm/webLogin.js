...
class QrCode extends St.Bin {
    ...
    update() {
        ...
        try {
            GnomeQR.generate_qr_code_async(
                ...
                this._cancellable,
                (_source, result) => this._onQrCodeGenerated(result));
        } catch (e) {
            logErrorUnlessCancelled(e, 'Failed to start QR code generation');
        }
    }
    ...
    _onQrCodeGenerated(result) {
        try {
            const [pixelData, qrSize] = GnomeQR.generate_qr_code_finish(result);
