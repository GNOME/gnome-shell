...
export const LoginDialog = GObject.registerClass({
    ...
    _createAuthMenuButton() {
        ...
        for (const id of ids) {
            let [sessionName, _] = Gdm.get_session_name_and_description(id);
    ...
    _onPrompted() {
        this._authMenuButton.updateItemsSensitivity(
            {sectionName: _SESSION_TYPE_SECTION_NAME},
            this._shouldShowSessionsList());
        ...
        for (const m of mechanisms) {
            if (GdmUtil.isSelectable(m)) {
                ...
                this._authIndicatorButton.addItem({
                    ...
                    description: this._getLoginDescription(m),
                    ...m,
                });
            }
        }
    ...
    _getLoginDescription(mechanism) {
        ...
        case GdmConst.FINGERPRINT_ROLE_NAME:
            return _('Log in with fingerprint');
        default:
