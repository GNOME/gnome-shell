...
export const AuthMenuButton = GObject.registerClass({
    ...
    _init(params) {
        ...
        this._descriptionLabel = new St.Label({
            style_class: 'login-dialog-auth-menu-description',
            y_align: Clutter.ActorAlign.CENTER,
            visible: false,
        });
    ...
    updateDescriptionLabel() {
        if (this._readOnly && this._items.size === 1) {
    ...
    updateItemsSensitivity(searchCriteria, visible) {
        const itemKeys = this._findItems(searchCriteria);
        ...
        for (const itemKey of itemKeys) {
            ...
            if (menuItem) {
                ...
                menuItem.reactive = visible;
            }
        }

        if (searchCriteria.sectionName) {
            const header = this._headers.get(searchCriteria.sectionName);
            if (header) {
                header.visible = visible;
                header.reactive = visible;
            }
        }
    }
