...
//
// FIXME: This doesn’t account for fast user switching. We can add support for
// it once malcontent can notify of changes to parental controls policies. See:
...
var ParentalControlsManager = class {
    constructor() {
        ...
        Malcontent.get_app_filter_async(null, Shell.util_get_uid (), false,
                                        null, this._onGetAppFilter.bind(this));
    }

    _onGetAppFilter(object, res) {
        try {
            this._appFilter = Malcontent.get_app_filter_finish(res);
        } catch (e) {
        ...
        this._appFilter.connect('app-filter-changed', () => {
            this.emit('changed');
        });
