...
export const TimeLimitsManager = GObject.registerClass({
    ...
    _updateState() {
        ...
        // Work out estimated times for the malcontent session limits calculations.
        const [, currentSessionStart, currentSessionEnd, nextSessionStart] = this._estimatedTimes;
        const malcontentSessionLimitsEnabled = this._parentalControlsManager.sessionLimitsEnabled();

        if (malcontentSessionLimitsEnabled) {
        ...
        const malcontentSessionLimitsDebug = malcontentSessionLimitsEnabled ? 'current session start: ' +
     ...
     * @type {boolean}
     */
    get malcontentSessionLimitsEnabled() {
        return this._parentalControlsManager.sessionLimitsEnabled();
    }
     ...
     *
     * Considers both the malcontent and wellbeing limits.
     *
     * @type {number}
     */
    get dailyLimitTime() {
    ...
    _onStateChanged() {
        ...
        case TimeLimitsState.ACTIVE: {
            if (this._manager.dailyLimitEnabled) {
                ...
                if (this._manager.wellbeingDailyLimitEnabled)
                    // Grayscale is in effect only when the wellbeing
        ...
        case TimeLimitsState.LIMIT_REACHED: {
            if (this._manager.malcontentSessionLimitsEnabled) {
        ...
        case TimeLimitsState.ACTIVE: {
            ...
            if (remainingSecs > LIMIT_UPCOMING_NOTIFICATION_TIME_SECONDS) {
                ...
                break;
                       ...
                       // Also check if we're late 1 second due to waiting for
                       // caching the estimated times for the parental controls
                       // session limits over D-Bus from the timer daemon.
                       Math.ceil(remainingSecs + 1) === LIMIT_UPCOMING_NOTIFICATION_TIME_SECONDS) {
