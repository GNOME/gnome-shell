				 ...
				 gpointer                          callback_data)
{
  ...
  if ((flags & NM_SECRET_AGENT_GET_SECRETS_FLAG_REQUEST_NEW) ||
      ((flags & NM_SECRET_AGENT_GET_SECRETS_FLAG_ALLOW_INTERACTION)
       && (self->priv->force_always_ask || is_connection_always_ask (request->connection))))
    {
                                  ...
                                  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_FORCE_ALWAYS_ASK:
      self->priv->force_always_ask = g_value_get_boolean (value);
      break;
                                  ...
                                  GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FORCE_ALWAYS_ASK:
      g_value_set_boolean (value, self->priv->force_always_ask);
      break;
