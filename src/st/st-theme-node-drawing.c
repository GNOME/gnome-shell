...
create_corner_material (StCornerSpec *corner)
{
  ...
  surface = cairo_image_surface_create_for_data (data,
                                                 ...
                                                 size, size,
                                                 rowstride);
  device_scaling = (float) size / logical_size;
...
st_theme_node_prerender_shadow (StThemeNodePaintState *state)
{
  ...
  state->box_shadow_width *= state->resource_scale;
  state->box_shadow_height *= state->resource_scale;

  /* Render offscreen */
  fb_width = ceilf (state->box_shadow_width);
  fb_height = ceilf (state->box_shadow_height);
  ...
  if (state->node != node ||
      st_theme_node_needs_new_box_shadow_for_size (state, node, width, height, resource_scale))
    {
      ...
      if (node->rendered_once && node->cached_textures &&
          ...
          fabsf (resource_scale - node->cached_state.resource_scale) < FLT_EPSILON)
