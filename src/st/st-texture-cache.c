...
/* Error quark */
GQuark
st_texture_cache_error_quark (void)
{
  return g_quark_from_static_string ("st-texture-cache-error-quark");
}
                                   ...
                                   gpointer             user_data)
{
  ...
  if (info == NULL)
    {
      g_task_return_new_error (task, ST_TEXTURE_CACHE_ERROR,
                               ST_TEXTURE_CACHE_ICON_LOOKUP_FAILED,
                               "GIcon lookup failed");
      g_object_unref (task);
      return;
    }
