...
static void
insert_in_file_scale_cache (GHashTable *scale_cache,
                            char       *key,
                              ...
                              const char *key)
{
  ...
  for (i = 0; i < scales->len; i++)
    {
      char *scaled_key = g_strdup_printf ("%s%f", key, g_array_index (scales, float, i));
      ...
      if (policy == ST_TEXTURE_CACHE_POLICY_FOREVER)
        {
          ...
          insert_in_file_scale_cache (cache->priv->keyed_file_scale_cache,
