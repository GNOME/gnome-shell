...
struct _StIconPrivate
{
  ...
  gboolean          fallback_icon_visible;
...
st_icon_init (StIcon *self)
{
  ...
  self->priv->fallback_icon_visible = FALSE;
  ...
  if (error != NULL)
    {
      if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
        {
          if (!priv->fallback_icon_visible)
            {
              /* If the request wasn't cancelled but failed for another reason,
               * first try again with the fallback gicon... */
              priv->fallback_icon_visible = TRUE;
              st_icon_update (self);
            }
          else
            {
              /* ...and if that didn't work, set the content to NULL. */
...
st_icon_update (StIcon *icon)
{
  ...
  clutter_actor_set_size (priv->icon_actor, priv->icon_size * paint_scale, priv->icon_size * paint_scale);
  ...
  gicon = priv->fallback_icon_visible ? priv->fallback_gicon : priv->gicon;
  ...
  st_texture_cache_load_gicon_async (cache, theme_node, gicon,
                                     priv->icon_size,
...
st_icon_set_gicon (StIcon *icon, GIcon *gicon)
{
  ...
  g_set_object (&priv->gicon, gicon);
  g_object_notify_by_pspec (G_OBJECT (icon), props[PROP_GICON]);

  /* Try showing the non-fallback gicon again */
  priv->fallback_icon_visible = FALSE;
  st_icon_update (icon);
}
