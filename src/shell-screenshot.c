  ...
  if (priv->filename != NULL) {
    if (callback)
      g_task_report_new_error (screenshot,
                               ...
                               G_IO_ERROR,
                               G_IO_ERROR_PENDING,
                               "Only one screenshot operation at a time is permitted");
    return;
  }
 ...
 * @filename_used: (out) (transfer none): the name of the file the screenshot was written to
...
shell_screenshot_screenshot_finish (ShellScreenshot        *screenshot,
                                    ...
                                    GError                **error)
{
  g_return_val_if_fail (g_async_result_is_tagged (result, shell_screenshot_screenshot), FALSE);
...
shell_screenshot_screenshot_area_finish (ShellScreenshot        *screenshot,
                                         ...
                                         GError                **error)
{
  g_return_val_if_fail (g_async_result_is_tagged (result, shell_screenshot_screenshot_area), FALSE);
  return finish_screenshot (screenshot, result, area, filename_used, error);
}
...
shell_screenshot_screenshot_window_finish (ShellScreenshot        *screenshot,
                                           ...
                                           GError                **error)
{
  g_return_val_if_fail (g_async_result_is_tagged (result, shell_screenshot_screenshot_window), FALSE);
  return finish_screenshot (screenshot, result, area, filename_used, error);
}
                                    ...
                                    GError          **error)
{
  ...
  g_return_val_if_fail (g_async_result_is_tagged (result, shell_screenshot_pick_color), FALSE);
  ...
  if (color)
    {
      uint8_t *data = (uint8_t *)cairo_image_surface_get_data (priv->image);
