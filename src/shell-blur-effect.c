...
shell_blur_effect_pre_paint (ClutterEffect *effect)
{
  ...
  if (!update_blur_fbo (effect, &self->blur[VERTICAL], width, height) ||
      !update_blur_fbo (effect, &self->blur[HORIZONTAL], width, height) ||
      !update_cache_fbo (effect, width, height))
    {
      return FALSE;
    }
