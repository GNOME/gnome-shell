 ...
 * blur-radiuses that allow downscaling the texture to smaller sizes, at
 * small radiuses where no downscaling is possible this can easily halve the
 * framerate.
 *
 * ## Multipass
 *
...
/* Sample sigma * 3 pixels in each direction, this is the factor used by most
 * implementations since it covers a high percentage of the gaussian shape. */
"  int n_steps = int (ceil (3 * sigma));                            \n"
"                                                                                \n"
/* Calculate the kernel values incrementally, see K. Turkowski,
 * "Incremental Computation of the Gaussian", GPU Gems 3, chapter 40:
...
"                                                                                \n"
 /* Fetch the current texel and apply the coefficient */
"  float gauss_coefficient_total = gauss_coefficient.x;                          \n"
"  vec4 ret = texture2D (cogl_sampler, uv) * gauss_coefficient.x;                \n"
"  gauss_coefficient.xy *= gauss_coefficient.yz;                                 \n"
"                                                                                \n"
/* Fetch the other texels and apply their coefficients, we can skip every
 * second texel here since we use the GPUs hardware interpolation to get an
 * interpolated value for two adjacent texels. */
            ...
            float            downscale_factor)
{
  ...
  unsigned int new_width = width / downscale_factor;
  unsigned int new_height = height / downscale_factor;
