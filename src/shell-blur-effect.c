 ...
 * # Modes
 *
 * #ShellBlurEffect can work in @SHELL_BLUR_MODE_BELOW and @SHELL_BLUR_MODE_ACTOR
 * modes. The actor mode blurs the actor itself, and all of its children. The
 * below mode blurs the pixels beneath the actor, but not the actor itself.
 *
 * @SHELL_BLUR_MODE_BELOW can be computationally expensive, since the contents
  ...
  if (G_UNLIKELY (brightness_pipeline == NULL))
    {
      ...
      brightness_pipeline = cogl_pipeline_new (ctx);
      ...
      cogl_pipeline_set_layer_null_texture (brightness_pipeline, 0);
      cogl_pipeline_set_layer_filters (brightness_pipeline,
                                       0,
                                       COGL_PIPELINE_FILTER_LINEAR,
                                       COGL_PIPELINE_FILTER_LINEAR);
    }
...
static gboolean
update_overlay_fbo (ShellBlurEffect *self,
                    unsigned int     width,
                    unsigned int     height)
{
  if (self->tex_width == width &&
      self->tex_height == height &&
      self->overlay_fb.framebuffer)
    {
      return TRUE;
    }

  return update_fbo (&self->overlay_fb, width, height, 1.0);
}
               ...
               ClutterPaintContext *paint_context)
{
  ...
  cogl_framebuffer_draw_rectangle (framebuffer,
                                   self->blur[HORIZONTAL].data.pipeline,
                                   0, 0,
                                   self->tex_width,
                                   self->tex_height);
...
static gboolean
paint_overlay (ShellBlurEffect     *self,
               ClutterPaintContext *paint_context)
{
  ...
  cogl_pipeline_set_color4ub (self->overlay_fb.pipeline,
                              paint_opacity,
                              paint_opacity,
                              paint_opacity,
                              paint_opacity);
  ...
  clear_framebuffer (self->overlay_fb.framebuffer);
...
update_framebuffers (ShellBlurEffect *self)
{
  ...
  if (self->mode == SHELL_BLUR_MODE_BELOW)
    updated |= update_overlay_fbo (self, width, height);
...
static gboolean
paint_actor (ShellBlurEffect     *self,
             ClutterPaintContext *paint_context)
{
  gboolean painted_offscreen = FALSE;
      ...
      if (needs_repaint (self, flags))
        {
          ...
          switch (self->mode)
            {
            case SHELL_BLUR_MODE_ACTOR:
              if (paint_actor (self, paint_context))
                apply_blur (self, paint_context, &self->actor_fb);
              break;

            case SHELL_BLUR_MODE_BELOW:
              paint_overlay (self, paint_context);
              apply_blur (self, paint_context, &self->overlay_fb);
              clutter_actor_continue_paint (self->actor, paint_context);
              break;
            }
        }
...
shell_blur_effect_finalize (GObject *object)
{
  ...
  clear_framebuffer_data (&self->actor_fb);
  clear_framebuffer_data (&self->overlay_fb);
  clear_framebuffer_data (&self->brightness_fb);
  clear_framebuffer_data (&self->blur[VERTICAL].data);
  clear_framebuffer_data (&self->blur[HORIZONTAL].data);

  g_clear_pointer (&self->actor_fb.pipeline, cogl_object_unref);
  g_clear_pointer (&self->overlay_fb.pipeline, cogl_object_unref);
...
shell_blur_effect_init (ShellBlurEffect *self)
{
  ...
  self->overlay_fb.pipeline = create_base_pipeline ();
                            ...
                            ShellBlurMode    mode)
{
  ...
  switch (mode)
    {
    case SHELL_BLUR_MODE_ACTOR:
      clear_framebuffer (&self->overlay_fb);
      break;
