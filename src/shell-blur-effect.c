...
static void
clear_cache_framebuffer (ShellBlurEffect *self)
{
...
static void
setup_projection_matrix (ShellBlurEffect *self,
                         float            width,
                         float            height,
                         float            downscale_factor)
{
  ...
  cogl_framebuffer_set_projection_matrix (self->blur[VERTICAL].framebuffer,
                                          &projection);
  cogl_framebuffer_set_projection_matrix (self->blur[HORIZONTAL].framebuffer,
                                          &projection);
  cogl_framebuffer_set_projection_matrix (self->framebuffer, &projection);
}
           ...
           ClutterPaintContext *paint_context)
{
  ...
  setup_projection_matrix (self, width, height, downscale_factor);

  clear_framebuffer (vblur->framebuffer);
            ...
            ClutterPaintContext *paint_context)
{
  ...
  clear_framebuffer (hblur->framebuffer);
  ...
  clear_framebuffer (self->framebuffer);
  ...
  if (!self->framebuffer || (flags & CLUTTER_EFFECT_PAINT_ACTOR_DIRTY))
    {
      ...
      /* Only setup blur when we have an actual radius */
