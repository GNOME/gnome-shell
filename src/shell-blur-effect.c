                  ...
                  ClutterPaintContext *paint_context)
{
  ...
  clutter_actor_get_paint_box (self->actor, &box);
  ...
  if (self->mode == SHELL_BLUR_MODE_BACKGROUND)
    {
      ...
      clutter_actor_box_set_origin (&box,
                                    box.x1 - stage_view_layout.x,
                                    box.y1 - stage_view_layout.y);
      clutter_actor_box_scale (&box, box_scale_factor);
    }
                  ...
                  ClutterPaintContext *paint_context)
{
  ...
  clutter_actor_box_get_origin (&self->actor_box,
                                &transformed_x, &transformed_y);
  clutter_actor_box_get_size (&self->actor_box,
                     ...
                     ClutterPaintContext *paint_context)
{
  ...
  update_actor_box (self, paint_context);
  clutter_actor_box_get_size (&self->actor_box, &width, &height);
      ...
      if (needs_repaint (self, flags))
        {
           ...
           */
          if (!update_framebuffers (self, paint_context))
            goto fail;
          ...
          switch (self->mode)
            ...
            case SHELL_BLUR_MODE_BACKGROUND:
              if (!paint_background (self, paint_context))
                goto fail;
