...
"      ret += c * fy;                                              \n"
"                                                                  \n"
"      c = texture2D(cogl_sampler, uv - vec2(0.0, offset_y));      \n"
"      total += fy;                                                \n"
"      ret += c * fy;                                              \n"
"    }                                                             \n"
"  } else {                                                        \n"
"    for (int x = 0; x < half_radius; x++) {                       \n"
...
"      ret += c * fx;                                              \n"
"                                                                  \n"
"      c = texture2D(cogl_sampler, uv - vec2(offset_x, 0.0));      \n"
"      total += fx;                                                \n"
"      ret += c * fx;                                              \n"
"    }                                                             \n"
"  }                                                               \n"
...
shell_blur_effect_pre_paint (ClutterEffect *effect)
{
  ...
  if (!update_blur_fbo (effect, &self->blur[VERTICAL], width, height, resource_scale) ||
      !update_blur_fbo (effect, &self->blur[HORIZONTAL], width, height, resource_scale) ||
      !update_cache_fbo (effect, width, height, resource_scale))
    {
      return FALSE;
    }
  ...
  setup_projection_matrix (vblur->framebuffer, width, height);
  setup_projection_matrix (hblur->framebuffer, width, height);
  setup_projection_matrix (self->framebuffer, width, height);
